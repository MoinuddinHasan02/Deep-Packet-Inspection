    ...
    while (reader.readNextPacket(raw)) {
        ...
        if ((flow.app_type == AppType::UNKNOWN || flow.app_type == AppType::HTTP) &&
            ...
            if (payload_offset + 12 < raw.data.size()) {
               ...
               size_t payload_len = raw.data.size() - payload_offset;
                const char* payload_ptr = reinterpret_cast<const char*>(raw.data.data() + payload_offset);
                std::string payload_str(payload_ptr, payload_len);
                
                if (payload_str.compare(0, 4, "GET ") == 0 || payload_str.compare(0, 5, "POST ") == 0) {
                    size_t host_pos = payload_str.find("Host: ");
                    if (host_pos != std::string::npos) {
                        host_pos += 6;
                        size_t end_pos = payload_str.find("\r\n", host_pos);
                        if (end_pos != std::string::npos) {
                            std::string http_host = payload_str.substr(host_pos, end_pos - host_pos);
                            flow.sni = http_host;
                            flow.app_type = sniToAppType(http_host);
                        }
                    }
                }
            }
